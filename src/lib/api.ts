...
export const appointmentAPI = {
  getAppointments: async (): Promise<Appointment[]> => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        localStorage.setItem('doctorai_appointments', JSON.stringify(data));
        return data;
      }
    } catch (err) {
      console.warn('Backend unavailable, checking localStorage fallback');
    }
    const local = localStorage.getItem('doctorai_appointments');
  ...
  createAppointment: async (data: CreateAppointmentData): Promise<Appointment> => {
    ...
    try {
      if (token) {
        ...
        if (response.ok) {
          ...
          localStorage.setItem('doctorai_appointments', JSON.stringify([created, ...list]));
          window.dispatchEvent(new Event('doctorai_appointment_updated'));
          return created;
        }
      }
    } catch (err) {
    ...
    const local = localStorage.getItem('doctorai_appointments');
    const list = local ? JSON.parse(local) : [];
    localStorage.setItem('doctorai_appointments', JSON.stringify([fallbackApt, ...list]));
    window.dispatchEvent(new Event('doctorai_appointment_updated'));
    return fallbackApt;
  },
  ...
  cancelAppointment: async (id: string): Promise<void> => {
    ...
    const local = localStorage.getItem('doctorai_appointments');
    if (local) {
      const list = JSON.parse(local).filter((a: Appointment) => a.id !== id);
      localStorage.setItem('doctorai_appointments', JSON.stringify(list));
    }
    window.dispatchEvent(new Event('doctorai_appointment_updated'));
  }
};
